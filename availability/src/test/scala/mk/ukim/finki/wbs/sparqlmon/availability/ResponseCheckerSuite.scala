...
class ResponseCheckerSuite extends FunSuite {
  ...
  test("ask response body cannot be serialized to json") {
    ...
    assert(ResponseChecker.checkAskResponse(res).unsafeRunSync().isEmpty)
  }
  ...
  test("ask response true") {
    ...
    assert(ResponseChecker.checkAskResponse(res).unsafeRunSync().isDefined)
  }
  ...
  test("select response body cannot be serialized to json") {
    val res = Response[IO](Status.Ok).withEntity("<html></html>")
    assert(ResponseChecker.checkSelectResponse(res).unsafeRunSync().isEmpty)
  }
  ...
  test("select response sparql") {
    ...
    assert(ResponseChecker.checkSelectResponse(res).unsafeRunSync().isDefined)
  }
}
